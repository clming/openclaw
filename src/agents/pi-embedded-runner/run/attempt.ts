...
function inferToolNameFromToolCallId(
  ...
  // Synthetic fallback IDs don't encode tool names; skip inference.
  if (isSyntheticAutoId(id)) {
    return null;
  }
...
// Detect synthetic fallback IDs generated by normalizeToolCallIdsInMessage
// (e.g. "call_auto_1", "callauto", "CallAuto3"). These should never be
// dispatched as tool names — they leak when models echo back fallback IDs.
function isSyntheticAutoId(value: string): boolean {
  return /^call_?auto_?\d*$/i.test(value);
}
...
function normalizeToolCallNameForDispatch(
  ...
  // Synthetic auto-generated IDs (e.g. "call_auto_1", "callauto") should never
  // be dispatched as tool names. They leak when models echo back fallback IDs
  // instead of real tool names. Try to recover from the actual tool call ID;
  // otherwise clear to empty so dispatch does not invoke a nonexistent tool.
  if (isSyntheticAutoId(trimmed)) {
    return inferToolNameFromToolCallId(rawToolCallId, allowedToolNames) ?? "";
  }
