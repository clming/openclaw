...
function parseReasoningItem(value: unknown): Extract<InputItem, { type: "reasoning" }> | null {
  ...
  if (
    ...
    (record.type !== "reasoning" && !record.type.startsWith("reasoning."))
  ) {
    return null;
  }
  return {
  ...
  for (const item of response.output ?? []) {
    if (item.type === "message") {
      ...
      if (
        ...
        (itemType !== "reasoning" && !itemType.startsWith("reasoning."))
      ) {
        continue;
      }
      ...
      const reasoningId = toNonEmptyString((item as { id?: unknown }).id);
      content.push({
        type: "thinking",
        thinking: reasoning,
        ...(reasoningId
