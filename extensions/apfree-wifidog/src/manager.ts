...
export class ApFreeWifidogBridge {
  private readonly config: ResolvedApFreeWifidogConfig;
  ...
  private nextReqId = Math.floor(Math.random() * 1_000_000_000);
  ...
  private resolveConnectedDeviceId(deviceIdOrAlias: string): string {
    ...
    for (const [id, alias] of this.deviceAliases.entries()) {
      if (alias.toLowerCase() === candidate.toLowerCase()) {
        return id;
      }
    }
    return candidate;
  ...
  async callDevice(params: {
    ...
    let deviceId = this.resolveConnectedDeviceId(params.deviceId);
    ...
    return await new Promise<JsonRecord>((resolve, reject) => {
      ...
      if (expectResponse) {
        const timer = setTimeout(() => {
          ...
          this.logger.warn(
            `apfree-wifidog: request timeout device=${deviceId} op=${op} req_id=${reqKey} socket_state=${session.socket.readyState}`,
          );
  ...
  private generateReqId(): number {
    // Device-side parser requires req_id to be a JSON integer.
    this.nextReqId += 1;
    if (this.nextReqId > 2_147_483_647) {
      this.nextReqId = 1;
    }
  ...
  private generateReqIdForDevice(deviceId: string): number {
    while (true) {
  ...
  private async handleMessage(
    ...
    if (op === "connect" || op === "heartbeat") {
      ...
      if (priorDeviceId && priorDeviceId !== deviceId) {
        const priorSession = this.sessions.get(priorDeviceId);
      ...
      // Create AWAS proxy for this device and forward the message
